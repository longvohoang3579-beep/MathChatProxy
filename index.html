...
<script>
// ============================================================
// ⚠️ CÁC HÀM VÀ LOGIC API ĐƯỢC THÊM VÀO (Đã Fix lỗi undefined) ⚠️
// ============================================================
...
// Hàm chung: Thêm tin nhắn người dùng (để hiển thị những gì họ đã nhập)
function appendUserMessage(boxId, message, imageBase64 = null) {
    ...
    if (imageBase64) {
        ...
        // Ảnh không nằm trong bubble, mà nằm ngay dưới bubble ảnh
        const imgContainer = document.createElement('div');
        imgContainer.className = 'flex justify-end';
        imgContainer.style.maxWidth = '80%';
        imgContainer.style.alignSelf = 'flex-end';
...
// Hàm chung: Thêm tin nhắn đang chờ phản hồi (để hiển thị trạng thái loading)
function appendAIPendingMessage(boxId, text = "AI đang xử lý...") {
    const chatBox = document.getElementById(boxId);
    let pendingMsg = document.getElementById('pending-ai-response');
    
    if (!pendingMsg) {
        pendingMsg = document.createElement('div');
        pendingMsg.id = 'pending-ai-response';
        pendingMsg.className = 'bubble ai-bubble';
        chatBox.appendChild(pendingMsg);
    }
    
    pendingMsg.innerHTML = `<span style="font-style:italic;">${text}</span>`;
    chatBox.scrollTop = chatBox.scrollHeight;
    return pendingMsg;
}
...
// Hàm RENDER GIF (Cần thiết cho Tạo Video)
...
async function createImage() {
    ...
    try {
        ...
        // FIX LỖI: Kiểm tra cả data.imageUrl và lỗi từ server
        if (response.ok && data.imageUrl) {
            ...
            const errorMsg = data.message || data.imageUrl || "❌ Lỗi không xác định khi tạo ảnh.";
            chatBox.appendChild(createErrorBubble(errorMsg));
        }
    } catch (error) {
        pendingMsg.remove();
        document.getElementById('imageBox').appendChild(createErrorBubble("❌ Lỗi kết nối đến Server (Network Error)."));
    }
    document.getElementById('imageBox').scrollTop = document.getElementById('imageBox').scrollHeight;
}

// 💬🧮🎞️ CHỨC NĂNG GỬI CHUNG (Chat, Math, Video) (API)
async function sendMessage(inputElementId, boxId, apiPath, isMath = false, isVideo = false) {
    ...
    // 1. Thêm tin nhắn/ảnh người dùng và reset input/attachment
    ...
    try {
        ...
        // FIX LỖI: Kiểm tra data.response (Chat/Math) và data.frames (Video)
        if (response.ok) {
            ...
            if (isVideo && data.frames && data.frames.length > 0) {
                ...
                if (isMath) {
                 ...
                 // Xử lý lỗi từ server (thường là Pollinations/Gemini lỗi)
                chatBox.appendChild(createErrorBubble(data.message));
            } else {
                 // Trường hợp phản hồi hợp lệ nhưng không có trường data.response/frames
                chatBox.appendChild(createErrorBubble("❌ Server không gửi lại dữ liệu hợp lệ (Missing 'response' or 'frames' field)."));
            }


        } else {
            // Lỗi HTTP (400, 500,...)
            const errorMsg = data.message || `❌ Lỗi HTTP ${response.status} từ Server.`;
            chatBox.appendChild(createErrorBubble(errorMsg));
        }
    } catch (error) {
        pendingMsg.remove();
        document.getElementById(boxId).appendChild(createErrorBubble("❌ Lỗi kết nối đến Server (Network Error)."));
    }
...
// ============================================================
// LOGIC KHỞI TẠO VÀ LẮNG NGHE SỰ KIỆN GỐC
// ============================================================

// Hiệu ứng mở đầu (Được giữ nguyên)
function playIntro() {
